...
static esp_err_t streamFile(httpd_req_t* req, const char* filepath, bool isGz) {
    ...
    // Set headers - use keep-alive for better performance
    httpd_resp_set_hdr(
        req,
        "Connection",
        "keep-alive");  // Use keep-alive instead of the close. This allows for fewer TCP messages to be transmitted.
...
static esp_err_t serveFile(httpd_req_t* req, const char* uri) {
    ...
    ESP_LOGE(TAG, "File not found: %s (SPIFFS: %s | SPIFFS.gz: %s | SD: %s)", uri, filepath, gzpath, sdpath);
    return ESP_FAIL;
}
...
static esp_err_t root_handler(httpd_req_t* req) {
    ...
    const char* fallback =
        ...
        "</head><body>"
        "<h1>SmartBike</h1>"
        "<p>index.html not found in SPIFFS. Put your site in the project's <code>data/</code> folder and run the PlatformIO task: Upload Filesystem "
        "Image.</p>"
...
static esp_err_t api_live_handler(httpd_req_t* req) {
    ...
    httpd_resp_set_hdr(req, "Keep-Alive", "timeout=5, max=1000");
...
static esp_err_t api_runs_in_handler(httpd_req_t* req) {
    ...
    char base[256];
    if (strcmp(runner, ".") == 0)
        snprintf(base, sizeof(base), "/sdcard");
    else
        snprintf(base, sizeof(base), "/sdcard/%s", runner);
...
static esp_err_t api_create_runner_handler(httpd_req_t* req) {
    ...
    char name[64] = { 0 };
    sscanf(np, "name=%63[^&]", name);
    if (!is_safe_name(name)) {
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "bad name");
        return ESP_FAIL;
    }
    char dir[160];
    snprintf(dir, sizeof(dir), "/sdcard/%s", name);
    struct stat st;
...
static esp_err_t api_rename_run_handler(httpd_req_t* req) {
    ...
    char* fp = strstr(buf, "folder=");
    char* op = strstr(buf, "old=");
    char* np = strstr(buf, "new=");
    if (!fp || !op || !np) {
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "missing params");
        return ESP_FAIL;
    }
    sscanf(fp, "folder=%99[^&]", folder);
    sscanf(op, "old=%99[^&]", oldname);
    sscanf(np, "new=%99[^&]", newname);
    // decode %2F
    for (char* p = folder; *p; ++p)
        if (*p == '%' && p[1] == '2' && (p[2] == 'F' || p[2] == 'f')) {
            *p = '/';
            memmove(p + 1, p + 3, strlen(p + 3) + 1);
        }
    for (char* p = oldname; *p; ++p)
        if (*p == '%' && p[1] == '2' && (p[2] == 'F' || p[2] == 'f')) {
            *p = '/';
            memmove(p + 1, p + 3, strlen(p + 3) + 1);
        }
    ...
    char oldPath[256];
    snprintf(oldPath, sizeof(oldPath), "/sdcard/%s/%s", folder, oldname);
    char newPath[256];
    snprintf(newPath, sizeof(newPath), "/sdcard/%s/%s", folder, newname);
...
static esp_err_t api_delete_run_handler(httpd_req_t* req) {
    ...
    char* fp = strstr(buf, "folder=");
    char* ffp = strstr(buf, "file=");
    if (!fp || !ffp) {
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "missing params");
        return ESP_FAIL;
    }
    sscanf(fp, "folder=%99[^&]", folder);
    sscanf(ffp, "file=%99[^&]", file);
    for (char* p = folder; *p; ++p)
        if (*p == '%' && p[1] == '2' && (p[2] == 'F' || p[2] == 'f')) {
            *p = '/';
            memmove(p + 1, p + 3, strlen(p + 3) + 1);
    ...
    if (strcmp(folder, ".") != 0 && !is_safe_name(folder)) {
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "bad folder");
        return ESP_FAIL;
    }
...
static esp_err_t api_active_runner_get(httpd_req_t* req) {
    ...
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    char out[96];
    snprintf(out, sizeof(out), "{\"active\":\"%s\"}", g_active_runner);
    httpd_resp_send(req, out, HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
static esp_err_t api_active_runner_post(httpd_req_t* req) {
    ...
    if (!rp) {
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "missing runner");
        return ESP_FAIL;
    }
    char name[64] = { 0 };
    sscanf(rp, "runner=%63[^&]", name);
...
static esp_err_t api_calibration_get(httpd_req_t* req) {
    ...
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    char out[64];
    snprintf(out, sizeof(out), "{\"calib_mode\":%s}", g_calibration_mode ? "true" : "false");
    httpd_resp_send(req, out, HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
static esp_err_t api_calibration_post(httpd_req_t* req) {
    ...
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    char out[80];
    snprintf(out, sizeof(out), "{\"ok\":true,\"calib_mode\":%s}", g_calibration_mode ? "true" : "false");
    httpd_resp_send(req, out, HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
static uint8_t find_best_channel(void) {
        ...
        if (ap_list) {
            ...
            for (int i = 0; i < ap_count; i++) {
                ...
                if (ch >= 1 && ch <= 13) {
                    ...
                    int weight = (ap_list[i].rssi > -50) ? 3 : (ap_list[i].rssi > -70) ? 2 : 1;
    ...
    ESP_LOGI(
        TAG,
        "Channel interference scores: ch1=%d, ch6=%d, ch11=%d -> selected channel %d",
        channel_usage[1],
        channel_usage[6],
        channel_usage[11],
        best_channel);
...
static httpd_handle_t start_webserver(void) {
    ...
    config.recv_wait_timeout = 30;    // Timeout for receiving data (increased for stability)
    config.send_wait_timeout = 30;    // Timeout for sending data (increased for stability)
    config.keep_alive_enable = true;  // Enable HTTP keep-alive
    config.keep_alive_idle = 5;       // Seconds before sending keep-alive probe
    config.keep_alive_interval = 5;   // Seconds between keep-alive probes
    config.keep_alive_count = 3;      // Number of probes before closing connection

    ESP_LOGI(TAG, "Starting HTTP server on port %d", config.server_port);

    if (httpd_start(&server, &config) == ESP_OK) {
        // API endpoints first (more specific routes)
        httpd_uri_t api_run_start_uri = { .uri = "/api/run/start", .method = HTTP_POST, .handler = api_run_start_handler, .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_run_start_uri);

        httpd_uri_t api_run_stop_uri = { .uri = "/api/run/stop", .method = HTTP_POST, .handler = api_run_stop_handler, .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_run_stop_uri);

        httpd_uri_t api_live_uri = { .uri = "/api/live", .method = HTTP_GET, .handler = api_live_handler, .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_live_uri);

        httpd_uri_t api_runs_uri = { .uri = "/api/runs", .method = HTTP_GET, .handler = api_runs_handler, .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_runs_uri);

        httpd_uri_t api_run_file_uri = { .uri = "/api/runs/*", .method = HTTP_GET, .handler = api_run_file_handler, .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_run_file_uri);

        // Runners list
        httpd_uri_t api_runners_uri = { .uri = "/api/runners", .method = HTTP_GET, .handler = api_runners_handler, .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_runners_uri);

        // Runs in folder
        httpd_uri_t api_runs_in_uri = { .uri = "/api/runs-in", .method = HTTP_GET, .handler = api_runs_in_handler, .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_runs_in_uri);

        // Move run
        httpd_uri_t api_move_run_uri = { .uri = "/api/move-run", .method = HTTP_POST, .handler = api_move_run_handler, .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_move_run_uri);

        httpd_uri_t api_create_runner_uri = { .uri = "/api/create-runner",
                                              .method = HTTP_POST,
                                              .handler = api_create_runner_handler,
                                              .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_create_runner_uri);

        httpd_uri_t api_delete_runner_uri = { .uri = "/api/delete-runner",
                                              .method = HTTP_POST,
                                              .handler = api_delete_runner_handler,
                                              .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_delete_runner_uri);

        httpd_uri_t api_rename_run_uri = { .uri = "/api/rename-run", .method = HTTP_POST, .handler = api_rename_run_handler, .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_rename_run_uri);

        httpd_uri_t api_delete_run_uri = { .uri = "/api/delete-run", .method = HTTP_POST, .handler = api_delete_run_handler, .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_delete_run_uri);

        // Active runner GET/POST
        httpd_uri_t api_active_runner_get_uri = { .uri = "/api/active-runner",
                                                  .method = HTTP_GET,
                                                  .handler = api_active_runner_get,
                                                  .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_active_runner_get_uri);
        httpd_uri_t api_active_runner_post_uri = { .uri = "/api/active-runner",
                                                   .method = HTTP_POST,
                                                   .handler = api_active_runner_post,
                                                   .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_active_runner_post_uri);

        httpd_uri_t api_calibration_get_uri = { .uri = "/api/calibration-mode",
                                                .method = HTTP_GET,
                                                .handler = api_calibration_get,
                                                .user_ctx = NULL };
        httpd_register_uri_handler(server, &api_calibration_get_uri);

        httpd_uri_t api_calibration_post_uri = { .uri = "/api/calibration-mode",
                                                 .method = HTTP_POST,
        ...
        // /ls handler
        httpd_uri_t ls_uri = { .uri = "/ls", .method = HTTP_GET, .handler = ls_handler, .user_ctx = NULL };
        ...
        httpd_uri_t favicon_uri = { .uri = "/favicon.ico", .method = HTTP_GET, .handler = static_handler, .user_ctx = NULL };
        httpd_register_uri_handler(server, &favicon_uri);

        // Catch-all for static files (must be last)
        httpd_uri_t static_uri = { .uri = "/*", .method = HTTP_GET, .handler = static_handler, .user_ctx = NULL };
...
static void start_mdns_service(void) {
    ...
    mdns_txt_item_t serviceTxtData[] = { { "board", "esp32" }, { "u", "user" } };
